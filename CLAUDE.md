...
- `tools/`: `verify.cjs`, `check-brand.cjs`, `check-budget.cjs`,
  ...
  `generate-video.mjs` (the editions as films for YouTube, below),
...
- `brand/`: the social kit (X header, LinkedIn banner and logo, YouTube
  ...
  upload sheets from `tools/generate-video.mjs`: megabytes each, ignored by
  git, regenerated with `npm run video`.
...
8. **The Navigator**, drafted in chat on 2026-09-16 from Tom's Hardware's
   ...
   The Floor.
9. **The YouTube channel** (2026-09-24). The films, thumbnails, banner,
   avatar and upload sheets exist (`npm run video`, `npm run social`);
   the channel does not. The owner's part: create the channel under the
   brand's Google account and claim the handle (probe `@aedificare`
   first); upload `brand/youtube-banner-2560x1440.png` and
   `brand/avatar-800.png`; per film, paste the `.txt` sheet's title and
   description and set the licence dropdown to "Creative Commons -
   Attribution", which is the site's CC BY 4.0 and the only choice that
   agrees with it; the trailer is the home film; no tags, no cards, no
   end screens, no call to action, one playlist named Editions. Then the
   URL goes into `humans.txt` and the JSON-LD `sameAs`, both in this repo,
   and nowhere else on a surface. Decisions only the owner can make, in
   order of weight: **sound** (silent as now; a tone computed from the
   same equation, k=5/7/3 as harmonic ratios, which keeps "nothing is
   drawn" true for the ear; or a voice, which is a new brand decision and
   the day video-use becomes relevant); **cadence** (one film per edition
   on the day it is issued is the only cadence the code is the date
   allows; a Short is the same film, not a second cut); **whether a
   draft's film is ever rendered** (`--slug` allows it; nothing publishes
   it). Not decided by the builder: a channel description. The kit's no
   tagline and no "Aedificare is" rules apply to the About box; the
   record line from the home film ("Four editions · CC BY 4.0") is the
   only text that qualifies.
